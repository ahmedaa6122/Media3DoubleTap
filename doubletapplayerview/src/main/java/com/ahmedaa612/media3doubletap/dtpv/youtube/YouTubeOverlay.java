package com.ahmedaa612.media3doubletap.dtpv.youtube;
...
import com.ahmedaa612.media3doubletap.dtpv.DoubleTapPlayerView;
import com.ahmedaa612.media3doubletap.dtpv.PlayerDoubleTapListener;
import com.ahmedaa612.media3doubletap.dtpv.SeekListener;
import com.ahmedaa612.media3doubletap.dtpv.youtube.views.CircleClipTapView;
import com.ahmedaa612.media3doubletap.dtpv.youtube.views.SecondsView;

/**
 * Overlay for [DoubleTapPlayerView] to create a similar UI/UX experience like the official YouTube
 * Android app.
 *
 * <p>The overlay has the typical YouTube scaling circle animation and provides some configurations
...
public final class YouTubeOverlay extends ConstraintLayout implements PlayerDoubleTapListener {
    ...
    private void initializeAttributes() {
        if (attrs != null) {
            TypedArray a =
                    getContext().obtainStyledAttributes(attrs, R.styleable.YouTubeOverlay, 0, 0);
            ...
            setAnimationDuration(
                    (long) a.getInt(R.styleable.YouTubeOverlay_yt_animationDuration, 650));

            seekSeconds = a.getInt(R.styleable.YouTubeOverlay_yt_seekSeconds, 10);

            setIconAnimationDuration(
                    (long) a.getInt(R.styleable.YouTubeOverlay_yt_iconAnimationDuration, 750));
            ...
            setTapCircleColor(
                    a.getColor(
                            ...
                            ContextCompat.getColor(
                                    getContext(), R.color.dtpv_yt_tap_circle_color)));

            setCircleBackgroundColor(
                    a.getColor(
                            R.styleable.YouTubeOverlay_yt_backgroundCircleColor,
                            ContextCompat.getColor(
                                    getContext(), R.color.dtpv_yt_background_circle_color)));
            ...
            this.setIcon(
                    a.getResourceId(
                            R.styleable.YouTubeOverlay_yt_icon, R.drawable.ic_play_triangle));
     ...
     *
     * <p>Links the DoubleTapPlayerView to this view for recognizing the tapped position.
     *
     ...
     *
     * <p>Performs seekTo-calls on the ExoPlayer's Player instance.
     *
    ...
    */
    ...
    /**
     * Sets a listener to execute some code before and after the animation (for example UI changes
     * (hide and show views etc.))
     */
    ...
    public final int getCircleBackgroundColor() {
        return ((CircleClipTapView) findViewById(R.id.circle_clip_tap_view))
                .getCircleBackgroundColor();
    }

    private final void setCircleBackgroundColor(int value) {
        ((CircleClipTapView) findViewById(R.id.circle_clip_tap_view))
                .setCircleBackgroundColor(value);
    }
    ...
    /**
     * Duration of the circle scaling animation / speed in milliseconds. The overlay keeps visible
    ...
    /**
     * Size of the arc which will be clipped from the background circle. The greater the value the
     * more roundish the shape becomes
     */
    ...
    /**
     * One of the three forward icons which will be animated above the seconds indicator. The rewind
     * icon will be the 180° mirrored version.
     *
     * <p>Keep in mind that padding on the left and right of the drawable will be rendered which
    ...
    private final void setTextAppearance(int value) {
        TextViewCompat.setTextAppearance(
                ((SecondsView) findViewById(R.id.seconds_view)).getTextView(), value);
     ...
     *
     * <p>In case of you'd like to change some specific attributes of the TextView in runtime.
     */
    ...
    /**
     * Seeks the video to desired position. Calls interface functions when start reached
     * ([SeekListener.onVideoStartReached]) or when end reached ([SeekListener.onVideoEndReached])
     *
    ...
    private void forwarding() {
        ...
        seekToPosition(
                player != null
                        ? player.getCurrentPosition() + (long) (this.seekSeconds * 1000)
                        : null);
    }
    ...
    private void changeConstraints(boolean forward) {
        ...
        ConstraintLayout rootLayout = findViewById(R.id.root_constraint_layout);
        ...
        if (forward) {
            // In LTR, forward means moving right.
            if (!isRTL) {
                ...
                constraintSet.connect(
                        ...
                        ConstraintSet.END);
            } else { // In RTL, forward means moving left.
            ...
            // In LTR, backward means moving left.
            if (!isRTL) {
                constraintSet.clear(secondsView.getId(), ConstraintSet.END);
                constraintSet.connect(
                        secondsView.getId(),
                        ...
                        ConstraintSet.PARENT_ID,
                        ConstraintSet.START);
            } else { // In RTL, backward means moving right.
                constraintSet.clear(secondsView.getId(), ConstraintSet.START);
                constraintSet.connect(
                        secondsView.getId(),
                        ConstraintSet.END,
